import { decode, verifyIdToken } from '../src/jwt';
...
describe('jwt', () => {
  ...
  it('does not validate nonce is present when options.nonce is undefined', async () => {
    ...
    expect(() =>
      verifyIdToken({ ...verifyOptions, nonce: undefined, id_token })
    ).not.toThrow();
  });
  ...
  it('validates nonce', async () => {
    const id_token = await createJWT();
    expect(() =>
      verifyIdToken({ ...verifyOptions, id_token, nonce: 'wrong' })
    ).toThrow(
  ...
  it('validates azp is present when `aud` is an array with more than one item', async () => {
    ...
    expect(() =>
      verifyIdToken({ ...verifyOptions, id_token, aud: 'other_value' })
    ).toThrow(
  ...
  it('validate auth_time + max_age is in the future', async () => {
    ...
    expect(() =>
      verifyIdToken({ ...verifyOptions, id_token, max_age: maxAge, leeway })
    ).toThrow(
